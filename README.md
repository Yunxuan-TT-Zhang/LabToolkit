...
A server is **required** for two things: installing TheLabToolkit as an app (the service
...
TheLabToolkit is a Progressive Web App: `manifest.webmanifest` + `sw.js` + `icons/`.
...
Paste (or photograph) a buffer description and TheLabToolkit parses each line into a
